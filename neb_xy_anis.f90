...
program neb
  ...
  do i=1, 15 !*** En realidad esto sera un  while ()
...
real*8 function energy_(nd_,p)  
  ...
  !via exchange ferromagnetico con J=1 y anisotropia en x con K=4J
  energy_=0
  do i=1,nd_-1 ! Termino de exchange
     energy_=energy_+((cos(p(i))*cos(p(i+1)))+(sin(p(i))*sin(p(i+1))))
  enddo
  do i=1,nd_ ! Termino de anisotropia
     energy_=energy_+(4*(cos(p(i))**2))
  enddo
  energy_=-energy_
...
subroutine set_initial_path (nd,ni,pth)
  ...
  do i=2,ni+1
     do j=1,nd
        pth(j,i)=pth(j,1)+((i-1)*dx(j)) + ((dx(j)/6)*(-1)**(j+i)) !+0.6 *(1.5-j)
                                                   !+((dx(j)/6)*(-1)**(j+i))! SE ESTA CORROMPIENDO LA TRAYECTORIA
...
subroutine vverlet (ndim,nparts,position,tvec)
  ...
  real*8 dt          ! time step
  parameter(mass=0.001,be=10,dt=1.0e-5)
...
subroutine compute(np,nd,pos,vel,mass,f,b,tv)
  ...
  do i=2,np+1
     ...
     !interactuando via exchange ferromagnetico con J=1 y anisotropia en x con K=4*J
     grad=0
     do k=1,nd
        ...
        grad(k)=-grad(k)  ! ...hasta aqui el termino de exchange        
        grad(k)=grad(k)+(2*4*cos(pos(k,i))*sin(pos(k,i)))
